...
/**
 * Field holding an string enumeration, of which we can
 * pick only one.
 *
 * @param{FieldDescr} f
 * @returns{FieldHTMLElement}
 */
function mkenumstr(f) {
	...
	p.rst = function() {
		if (f.options !== undefined && Object.keys(f.options).length > 0)
			p.set(f.options[0].value);
	}
...
/**
 * Field holding a number enumeration, of which we can
 * pick only one.
 *
 * @param{FieldDescr} f
 * @returns{FieldHTMLElement}
 */
function mkenumnum(f) {
	let p = mkenumstr(f);
