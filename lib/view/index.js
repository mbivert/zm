...
function mkindex(S) {
	...
	// XXX/TODO: this triggers a page reload, because we
	// can't (?) detect a hash change when listening on
	// popstate (see '../spa.js:/"popstate' and
	// https://stackoverflow.com/q/25634422 )
	//
	...
	/** @param{string} s */
	function setinput(s) {  pinput.value = s; }
	...
	//
	// That's to say, the fact that this is called "build"
	...
	// Analyse default text ("你好，世界")
	if (!S.toanalyse && !S.tosearch && !S.tocontains) analyse();
