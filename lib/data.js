...
function getmetas(xs) {
	if (Config.embedded) {
		return getmetascache(xs)
	}

	return RPC.pcall("/get/metas", {
		"names" : xs,
...
/**
 * Retrieve meta-datas for the Datas in xs from a local
 * cache. This local cache is solely intended when zm is
 * embedded in an external website, and thus has no dedicated
 * backend.
 *
 * TODO: tests
 *
 ...
 * error message
 */
function getmetascache(xs) {
	let ys = xs.reduce(
		/**
		 * @param{Object.<string, boolean>} acc
		 * @param{string}                   x
	...
	return Promise.resolve(DB.datas.reduce(
		/**
		 * @param{Datas} acc
		 * @param{any}                   data
		 * @returns{Datas}
		 */
		(acc, data) => {
			if (data.Name in ys) acc.push({
				"type": data.Type,
				"name": data.Name,
				"fmt":  data.Fmt,
				"file": data.File,

				// TODO: /get/metas/ only returns the fields above.
				// yet, those are declared for Data. It may be a leftover
				// from when we had no backend.
				"descr": "",
				"fmtparams": "",
				"id": -1,
				"license": "",
...
function parse(d, s) {
	...
	let [x, err] = parsers[d.type][d.fmt](s);
